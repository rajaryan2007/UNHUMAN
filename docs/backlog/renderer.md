...
- [ ] Compute feature — **STUB** — `VulkanComputePipeline.{h,cpp}` exists, is compiled, and is referenced from nowhere; no dispatch path
...
- [ ] Render graph — **STUB** — `Platform/Vulkan/RenderGraph/RenderGraphVulkan.{h,cpp}` is an empty class (`Init/Execute/Cleanup` with no bodies), unreferenced; the real design is the framegraph doc
...
## Frame graph (not in #7, added here so it isn't lost)

- [ ] FrameGraph frontend over the RenderGraph backend — **DESIGN ONLY** — framegraph doc Phases 4–5
